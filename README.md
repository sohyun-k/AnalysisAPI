...
```bash
AnalysisAPI$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
AnalysisAPI$ sam local start-api
AnalysisAPI$ curl http://localhost:3000/
```
...
```bash
AnalysisAPI$ sam logs -n HelloWorldFunction --stack-name AnalysisAPI --tail
```
...
```bash
aws cloudformation delete-stack --stack-name AnalysisAPI
```
